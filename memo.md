...
(μTRON-4.02仕様書p37)。実際のヘッダファイルを見てみると、文字通りマクロのみを読み込む処理になっている。
...
### 見つけにくいAPIたち
+ **get_OS_flag**:
`ecrobot_body.c`に定義されている、`OS_flag`の値を取得する関数。
...
+ **check_NXT_buttons**:
`ecrobot_body.c`に定義されている、`ecrobot_poll_nxtstate`のラッパー。多分。
